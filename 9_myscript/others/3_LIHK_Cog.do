...
local vars "fl3 fl7 fl8 fl4 fl9"
merge m:1 fid12 using cfps2012famecon_201906, keepusing(`vars') keep(match) nogen 
foreach i in `vars' {
	replace `i' = 0 if `i' == -8
	replace `i' = . if `i' < 0 
}
...
*------ 2.3: 链接认知技能，生成交互项
cd "$mydir\cognitive"
merge 1:1 cyear pid using 1_Cog, nogen keep(match) keepusing(cog cog_group)
gen cog_wy = cog * wy
label var cog_wy "认知技能与省劳均GDP的交乘项"
gen cog_indus = cog * indus
...
forvalues i = 2010(2)2020 {
	...
	foreach a in 1 0 {
		foreach b in 1 0 {
			...
			di("*--------------------------------------------------------*")
			local vars "Linc eduy cog exp exp2"
...
*------ 2.3: 对全样本回归
	local vars "cons eduy eduy_wy eduy_indus cog cog_wy cog_indus exp exp2 avwage"
	foreach v in `vars' {
		gen b_`v' = .
	}
	gen a = .
	...
	di("*--------------------------------------------------------*")
	reg Linc eduy cog exp exp2 

	// 增加认知技能交互项估计
	di _newline(3)
	di("*--------------------------------------------------------*")
	di "reg Linc avwage cog cog_wy cog_indus exp exp2"	
	di("*--------------------------------------------------------*")
	reg Linc avwage cog cog_wy cog_indus exp exp2
	...
	di _newline(3)
	di("*--------------------------------------------------------*")
	di "reg Linc avwage eduy eduy_wy eduy_indus cog cog_wy cog_indus exp exp2"	
	di("*--------------------------------------------------------*")
	reg Linc avwage eduy eduy_wy eduy_indus cog cog_wy cog_indus exp exp2 
...
*--------- 2.4.1: 教育、经验、认知
	...
	foreach a in 0 1 {     
		foreach b in 0 1 {
			di "urban = "`a'
			di "gender = "`b'
			reg Linc eduy cog exp exp2 if urban == `a' & gender == `b'
			sum Linc eduy cog exp exp2 if urban == `a' & gender == `b'
			predict ooo, xb    
			...
			local vars "avwage eduy eduy_wy eduy_indus cog cog_wy cog_indus exp exp2"
			foreach v in `vars' {
...
*------ 2.5: 基于全国样本估计出的系数，在省级调整
	...
	gen idx_cog = b_cog + b_cog_wy * wy / 1000 + b_cog_indus * indus
	gen idx_exp = b_exp
	gen idx_exp2 = b_exp2

	keep cyear provcd age urban gender eduy cog* exp* idx* intercept
...
keep cyear provcd urban gender age eduy cog* exp* idx* int* 
...
qui reg cog cyear cyear2 if id==1
gen tem1=_b[cyear]
gen tem2=_b[cyear2]
gen tem3=_b[_cons]
gen tem4=tem1*cyear+tem2*cyear2+tem3 if id==1
gen idx_cog_1=.
replace idx_cog_1 = tem4 if id==1
drop tem*
...
forvalues i = 2/100 {
	...
	qui reg cog cyear cyear2 if id==`i'
	gen tem1=_b[cyear]
	gen tem2=_b[cyear2]
	gen tem3=_b[_cons]
	gen tem4=tem1*cyear+tem2*cyear2+tem3 if id==`i'
...
gen lnh = eduy*idx_eduy_1 + cog*idx_cog_1 + exp*idx_exp_1 + exp2*idx_exp2_1
replace lnh = int01-int00 + eduy*idx_eduy_1 + cog*idx_cog_1 + exp*idx_exp_1 + exp2*idx_exp2_1 ///
	if urban == 0 & gender == 1
replace lnh = int10-int00 + eduy*idx_eduy_1 + cog*idx_cog_1 + exp*idx_exp_1 + exp2*idx_exp2_1 ///
	if urban == 1 & gender == 0
replace lnh = int11-int00 + eduy*idx_eduy_1 + cog*idx_cog_1 + exp*idx_exp_1 + exp2*idx_exp2_1 ///
	if urban == 1 & gender == 1

gen idx_h = exp(lnh)
label var idx_h "人力资本指数"

// 分年份、分省份、分城乡、性别、年龄、受教育分组、认知分组，取人力资本指数的均值，作为5分人口的平均人力资本指数
gen sch = 0
...
local vars "cyear provcd urban gender age sch cog_group"
...
cd "$mydir\macrodata"
local vars "cyear provcd urban gender age sch cog_group"
merge 1:1 `vars' using 4_LongPop5, nogen keep(match) 
sor `vars'
order `vars'
gen h = avg_idx_h * pop5
label var h "五分人口LIHK存量"
cd "$mydir\LIHK\1_LIHK"
...
bys cyear provcd: egen Pop = total()
cd "$mydir\LIHK\1_LIHK"
