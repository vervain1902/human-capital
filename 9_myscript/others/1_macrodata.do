...
Note:
	[4-fold population] meaning province/urban/gender/age group each year；
...
global popdir  	"$rawdir\CHLR\1_Laborpop_1985_2021\劳动力四分数据"
global macrodir "$rawdir\CHLR\2_Macrodata_1985_2021\2023宏观数据库"
...
foreach i in $provcds_ {
	foreach j in 0 1 {
		foreach k in 0 1 {
			...
			cd "$mydir\macrodata\3_Description\5_Pop4"
...
foreach i in $provcds_ {
	foreach j in 0 1 {
		...
		twoway line tpop cyear, xlabel(2010(2)2020) ///
			...
			subtitle("2010-2020") note("Data source: CHLR 2023") ///
			legend(ring(1) pos(6) cols(6) size(small)) 
		cd "$mydir\macrodata\3_Description\1_Pop2\1_Urban"
...
foreach i in $provcds_ 31 {
	foreach j in 0 1 {
		...
		twoway line tpop cyear, xlabel(2010(2)2020) ///
			...
			subtitle("2010-2020") note("Data source: CHLR 2023") ///
			legend(ring(1) pos(6) cols(6) size(small)) 
		cd "$mydir\macrodata\3_Description\1_Pop2\2_Gender"
...
foreach i in $provcds_ 31 {
	...
	twoway line pop0 cyear, xlabel(2010(2)2020) ///
		...
		subtitle("2010-2020") note("Data source: CHLR 2023") ///
		legend(ring(1) pos(6) cols(6) size(small)) 
	cd "$mydir\macrodata\3_Description\2_Pop0"
	...
	foreach j in 0 1 {
		foreach k in eduy2 aveduy2 {
			...
			twoway line `k' cyear, xlabel(2010(2)2020) ///
				...
				subtitle("2010-2020") note("Data source: CHLR 2023") ///
				legend(ring(1) pos(6) cols(6) size(small)) 
			cd "$mydir\macrodata\3_Description\3_EduYear2\1_Urban"
	...
	foreach j in 0 1 {
		foreach k in eduy2 aveduy2 {
			...
			twoway line `k' cyear, xlabel(2010(2)2020) ///
				...
				subtitle("2010-2020") note("Data source: CHLR 2023") ///
				legend(ring(1) pos(6) cols(6) size(small)) 
			cd "$mydir\macrodata\3_Description\3_EduYear2\2_Gender"
